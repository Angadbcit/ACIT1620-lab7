...
function activate() {
    for (let title of getTiles()) {
        title.toggleAttribute('disabled', false);
    }
...
function play() {
    ...
    const tiles = getTiles(); // -> node list
    const randomizedTiles = shuffle(Array.from(tiles)); // first turn node list into array
...
function pause() {
    deactivate();
    getOutput().querySelector('span:not(last-child)') .textContent = `You have ${--tries}`;
